...
void compileBlock(void) {
  ...
  if (lookAhead->tokenType == KW_CONST) {
    ...
    do {
      char *constName = lookAhead->string;
      eat(TK_IDENT);
      eat(SB_EQ);
      ConstantValue* value = compileConstant();
      eat(SB_SEMICOLON);
      
      Object* obj = createConstantObject(constName);
...
void compileBlock2(void) {
  ...
  if (lookAhead->tokenType == KW_TYPE) {
    ...
    do {
      char *typeName = lookAhead->string;
      eat(TK_IDENT);
      eat(SB_EQ);
      Type* type = compileType();
      eat(SB_SEMICOLON);
      
      Object* obj = createTypeObject(typeName);
...
void compileBlock3(void) {
  ...
  if (lookAhead->tokenType == KW_VAR) {
    ...
    do {
      char *varName = lookAhead->string;
      eat(TK_IDENT);
      eat(SB_COLON);
      Type* type = compileType();
      eat(SB_SEMICOLON);
      
      Object* obj = createVariableObject(varName);
...
void compileFuncDecl(void) {
  ...
  char *funcName = lookAhead->string;
  eat(TK_IDENT);
...
void compileProcDecl(void) {
  ...
  Object* proc;  
  eat(KW_PROCEDURE);  
  char *procName = lookAhead->string;
  eat(TK_IDENT);
...
void compileParam(void) {
  ...
  char *paramName = lookAhead->string;
  eat(TK_IDENT);
  eat(SB_COLON);
  type = compileBasicType();
  
  param = createParameterObject(paramName, paramKind, owner);
